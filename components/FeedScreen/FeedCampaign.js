...
import { Text, ImageBackground, ActivityIndicator, TouchableOpacity, FlatList, Platform } from 'react-native';
	...
	return (
		<View style={styles.container}>
			...
			<View>
				<TouchableOpacity activeOpacity={0.5} onPress={goToCampaign}>
					{data.camp_img.includes('.mov') || data.camp_img.includes('.mp3') || data.camp_img.includes('.mp4') ? (
						<View>
							{data.urgency ? (
						...
						<ImageBackground source={{ uri: data.camp_img }} style={styles.campImgContain}>
							{data.urgency ? (
								<View style={urgencyStyles}>
									<Text style={styles.urgencyBarText}>{urgencyStatus}</Text>
								</View>
        ...
        <View style={styles.bookmarks}>
          ...
          >
			...
			<View>
				{data.comments.length >= 1 ? data.comments.length === 1 ? (
