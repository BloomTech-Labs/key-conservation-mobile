...
const KEYS_TO_FILTERS = [ 'camp_name', 'camp_desc', 'username', 'location', 'data', 'update_desc' ];
...
class SearchScreen extends React.Component {
	...
	render() {
		...
		const filterCamps = this.props.allCampaigns.filter(createFilter(this.state.searchTerm, KEYS_TO_FILTERS));
		return (
				...
				<ScrollView stickyHeaderIndices={[ 0 ]}>
						...
						containerStyle={{
							...
							height          :
								Constants.deviceName === 'Iphone Xr' && Constants.deviceName === 'Iphone Xs'
									? null
									: (100 && Constants.deviceName === 'Iphone 7') || 'Iphone 8' || 'Pixel' || 'Pixel XL' ? null : 80,
						}}
						leftComponent={
							<TouchableOpacity
					...
					{this.props.allCampaigns.length > 0 &&
						filterCamps.map((camp) => {
							if (camp.update_id) {
								return <FeedUpdate key={`update${camp.update_id}`} data={camp} navigation={navigation} />;
							} else {
								return <FeedCampaign key={camp.camp_id} data={camp} navigation={navigation} />;
							}
						})}
				</ScrollView>
