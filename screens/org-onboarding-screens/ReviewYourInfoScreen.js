...
const ReviewYourInfoScreen = (props) => {
	...
	return (
		<KeyboardAvoidingView style={styles.container} behavior='height' keyboardVerticalOffset={86} enabled>
			<ScrollView style={styles.scrollView}>
				...
				<View>
					<View>
						<Text style={styles.obText}>Check that everything looks good and tap submit, or go back and edit</Text>
					</View>
					{!isEditingAccount ? (
						<View style={styles.borderContainer}>
							<View style={[ styles.row, styles.opaqueHeader ]}>
								<Text style={[ styles.obSubtitle, { marginRight: 20 } ]}>Account Administrator</Text>
						...
						<View style={styles.borderContainer}>
							...
							<View style={styles.row}>
								...
								<TextInput
									...
									onChangeText={(text) => setState({ ...state, point_of_contact: text })}
								/>
							</View>
							...
							<View style={styles.row}>
								...
								<TextInput
									...
									onChangeText={(text) => setState({ ...state, poc_position: text })}
								/>
							</View>
						</View>
					)}
					{!isEditingContact ? (
						<View style={styles.borderContainer}>
							<View style={[ styles.row, styles.opaqueHeader ]}>
								<Text style={[ styles.obSubtitle, { marginRight: 20 } ]}>Contact & Credentials</Text>
					...
					{!isEditingOrganization ? (
						<View style={styles.borderContainer}>
							<View style={[ styles.row, styles.opaqueHeader ]}>
								<Text style={[ styles.obSubtitle, { marginRight: 20 } ]}>Organization Activity</Text>
								<TouchableOpacity onPress={() => setIsEditingOrganization(!isEditingOrganization)}>
									<EditPencil />
								</TouchableOpacity>
							</View>
						...
						<View style={styles.borderContainer}>
							...
							<TextInput
								...
								onChangeText={(text) => setState({ ...state, other_countries: text })}
							/>
					...
					{!isEditingAffiliations ? (
						<View style={styles.borderContainer}>
							<View style={[ styles.row, styles.opaqueHeader ]}>
								<Text style={[ styles.obSubtitle, { marginRight: 20 } ]}>Partnerships & Affiliations</Text>
								<TouchableOpacity onPress={() => setIsEditingAffiliations(!isEditingAffiliations)}>
									<EditPencil />
								</TouchableOpacity>
							</View>
							{state.affiliations_partnerships.split(',').map((affilicated, idx) => {
						...
						<View style={styles.borderContainer}>
							...
							{state.affiliations_partnerships.split(',').map((affilicated, idx, arr) => {
								if (affilicated === '') return;
								return (
									<TextInput
										key={idx}
					...
					{!isEditingMisc ? (
						<View style={styles.borderContainer}>
							<View style={[ styles.row, styles.opaqueHeader ]}>
								<Text style={[ styles.obSubtitle, { marginRight: 20 } ]}>Miscellaneous Items</Text>
								<TouchableOpacity onPress={() => setIsEditingMisc(!isEditingMisc)}>
									<EditPencil />
								</TouchableOpacity>
							</View>

							{state.conservation_optimism && (
								<Text style={styles.obText}>We will join Key Conservation in practicing Conservation Optimism.</Text>
							)}
							{state.smartphone_access && <Text style={styles.obText}>Our Organization does use smartphones.</Text>}
						</View>
					) : (
						<View style={styles.borderContainer}>
							<View style={[ styles.row, styles.opaqueHeader ]}>
							...
							<View style={styles.column}>
								<Text style={styles.obText}>Will you join us in Conservation Optimism?</Text>
								<Switch
									trackColor={{ true: '#00FF9D' }}
									style={styles.obSwitchButton}
									value={state.conservation_optimism}
									onValueChange={(newValue) => setState({ ...state, conservation_optimism: newValue })}
								/>
							</View>
							<View style={styles.column}>
								<Text style={styles.obText}>Does your organization have access to a smartphone?</Text>
								<Switch
									style={styles.obSwitchButton}
									trackColor={{ true: '#00FF9D' }}
									value={state.smartphone_access}
									onValueChange={(newValue) => setState({ ...state, smartphone_access: newValue })}
								/>
							</View>
						</View>
					)}
